...
- 진입점: ~~생성 페이지(/generate) 상단에 "AI 지시문" 링크. 사이드바엔 넣지 않는다(저빈도).~~
  **개정(2026-08-10, 사용자 피드백)**: 사이드바에 "설정" 캡션 그룹을 신설해 클라이언트·AI 지시문을
  거기로 옮긴다. 원칙 — 매일 쓰는 작업 화면(콘텐츠 생성)과 가끔 손보는 설정 화면(재료·규칙)을
  그룹으로 분리하고, **앞으로 설정 성격 기능은 이 그룹에 추가**한다. /generate 상단 링크는 제거
  (사이드바로 이동), /generate의 "클라이언트 등록하러 가기" 빈 상태 안내는 유지(문맥 안내라 역할이 다름).
- 레이아웃 개정(2026-08-10, 사용자 피드백): 편집 필드(좌)·미리보기(우, sticky) 2단 — 수정→반영을
  한 시야에서. 변경 이력은 하단 전체 폭. max-w 720→1100px.
