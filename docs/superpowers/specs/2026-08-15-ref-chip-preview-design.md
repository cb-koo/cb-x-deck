...
- **시트의 번역 밴드도 본문 아래로 통일**(koo 확정 3차, 08-15): "실제 X의 자리"가 규칙 — RefTweetCard의
  translation prop 밴드를 지표 행 뒤에서 본문 아래로 이동, 스타일도 TweetCard 밴드로 통일. 시트만은
  카드별 토글 없음(상단 '전체 번역'이 토글). translation과 translateSlot은 상호 배타(주석 명시).
