...
`i`, `home`, `explore`, `search`, `notifications`, `messages`, `settings`, `compose`, `intent`, `share`, `hashtag`, `login`, `logout`, `signup`, `account`, `tos`, `privacy`, `about`, `download`.
...
`PATCH /api/columns/[id]` (`[id]/route.ts:21`~`36`): 기존 "핸들이 바뀌었을 때만 재해석" 규칙을 유지하되 **정규화된 값끼리 비교**한다. 지금은 `@hadakan__`↔`hadakan__`처럼 같은 계정을 가리키는 표기 차이가 "변경"으로 잡혀 불필요한 API 콜이 난다.

- `patch.config`에 handle 키가 없거나 빈 값이면 지금처럼 통과(폭·정렬만 바꾸는 PATCH를 막지 않는다).
- 값이 있는데 파싱 실패면 400.
- 정규화 결과가 기존 핸들과 **대소문자 무시 동일**이면 재해석을 건너뛰고 `patch.config.handle`을 **기존 값으로 되돌린다** — 건너뛴 경로에서 사용자가 친 임의 표기가 저장되면 안 된다.
...
- **성공 피드백**: 렌더마다 `parseXHandle(handle)`로 파생. `ok`면 `✓ @<handle> 을 추적할게요`. 숫자·값만 던지지 않고 결과를 서술한다(원칙 3). 파생값이라 라벨/값 불일치가 구조적으로 불가능하다(원칙 4).
