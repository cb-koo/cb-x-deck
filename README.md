...
- **인플루언서 배정**: 편집창에서 원고를 게시할 인플루언서를 지정한다(@핸들 또는 프로필 링크 붙여넣기 — 둘 다 핸들로 정규화). 배정 결과는 카드·테이블·칸반에 함께 보이고, 이미 배정한 적 있는 계정이 자동완성으로 제안된다. 핸들을 식별자로 저장하므로 향후 인플루언서 관리 기능이 이 값을 그대로 이어받는다
