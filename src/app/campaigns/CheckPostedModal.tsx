...
export function CheckPostedModal({ result, tasks, onClose, onMarkRemoved }: {
  ...
  const reasonOf = (s: CheckPostedResult['skipped'][number]) => s.reason === 'no_target' ? '대상 미정' : s.reason === 'no_handle' ? '인플루언서 미배정' : s.reason === 'target_cancelled' ? `대상 게시글(${targetOf(s.taskId)})이 취소됨` : `대상 게시글(${targetOf(s.taskId)})이 아직 게시 전`;
