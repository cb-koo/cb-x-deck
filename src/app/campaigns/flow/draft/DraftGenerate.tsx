...
export function DraftGenerate({
  task, clientId, clientData, quoteTarget, onAttached, onAttachFailed, onGenerated, onBusyChange, onOverlayChange, onDirtyChange,
}: {
  task: FlowRow;
  ...
  // tweetId는 보내지 않는다. 서버가 이 인용RT 작업의 최신 대상을 다시 읽어 스냅샷을 만든다.
  quoteTarget: { taskId: string; label: string; tweetId: string | null } | null;
  ...
  async function run() {
    ...
    const r = await createDraftsApi({
      ...
      ...(quoteTarget ? { quoteTargetTaskId: quoteTarget.taskId } : {}),
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        {quoteTarget && <p className="text-caption text-x-muted">{quoteTarget.tweetId
      ...
      {quoteTarget?.tweetId && <p className="text-caption text-x-muted">저장된 대상 본문이 없으면 X에서 불러와요. 게시물 조회 1회에 약 $0.001이 추가돼요.</p>}
