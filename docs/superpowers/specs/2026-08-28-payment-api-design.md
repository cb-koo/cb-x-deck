...
제약 재생성(`drop constraint if exists` + `add … not valid`, 040 관례). payload `{ requestId, amountGross, currency, taskType, paidAmountKrw }`. 문구 `지급 완료 · ¥3,158 → 실지급 ₩29,700`. 접수·지급 예정·보류는 타임라인에 남기지 않는다(배지로 충분).
...
- 접힌 줄: 상태 배지 라벨만 바뀐다. 둘째 줄 끝에 지급 완료면 `실지급 ₩29,700 (요청 ₩30,000, −300)`; 차이 0이면 `실지급 ₩30,000`.
- 펼침 `<dl>`에 `정산` 항목 1개(11항목 아래, 만든 사람 위):
  ...
  - 완료: `지급 완료 · 8-30 14:10 · 실지급 ₩29,700 (요청 ₩30,000, −300)` (+ `· 메모: {note}`)
