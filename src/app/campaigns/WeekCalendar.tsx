...
import { weekRows, calendarGrid, dateAnchorLabel } from '@/lib/campaignCalendar';
import { draftLabel } from '@/lib/draftViews';
import { STAGE_BAR_HEX, OVERDUE_BAR_HEX } from '@/components/DraftStatusChip';
...
// 카드를 끌어 칸에 놓으면 예정일이 바뀌고, 격자 카드를 미정 섹션에 놓으면 예정일이 지워진다 —
...
// 가독성: 카드 제목 15px(text-content)·보조 13px(text-ui)·날짜 앵커 13px. text-caption(11px)은 쓰지 않는다.
...
const barColor = (stage: ContentStage, overdue: boolean) => (overdue ? OVERDUE_BAR_HEX : STAGE_BAR_HEX[stage]);
// 범례 — 색만으로 단계를 말하지 않기 위해(색맹 접근성) 격자 아래에 점+이름을 함께 둔다.
const LEGEND: Array<{ hex: string; label: string }> = [
  { hex: STAGE_BAR_HEX.draft, label: STAGE_LABEL.draft },
  { hex: STAGE_BAR_HEX.review, label: STAGE_LABEL.review },
  { hex: STAGE_BAR_HEX.approved, label: STAGE_LABEL.approved },
  { hex: STAGE_BAR_HEX.delivered, label: STAGE_LABEL.delivered },
  { hex: STAGE_BAR_HEX.published, label: STAGE_LABEL.published },
  { hex: OVERDUE_BAR_HEX, label: '밀림' },
];

export function WeekCalendar({ rows, campaign, today, filter, onOpenDraft, onChangeScheduledOn }: {
  ...
  // 격자 뼈대는 전체 원고로 계산한다 — 필터를 바꿀 때마다 주 행이 생겼다 사라지면 같은 캠페인인데 달력 크기가 달라 보인다.
  const weeks = weekRows(campaign.startsOn, campaign.endsOn, rows.map((r) => r.scheduledOn));
  const grid = calendarGrid(rows.filter((d) => matchesStageFilter(d, filter)), weeks, today);
  const dragged = dragId ? rows.find((x) => x.id === dragId) ?? null : null;
  // 예정일 있는 카드를 끌고 있으면 미정 섹션이 비어 있어도 띠를 내보인다 — 놓을 곳이 없으면 '예정일 지우기'가 불가능해진다
  const showUnscheduled = grid.unscheduled.length > 0 || (dragged !== null && dragged.scheduledOn !== null);
  const inPeriod = (date: string) => !isOutOfRange(date, campaign.startsOn, campaign.endsOn);

  // drop 대상 — 날짜 문자열 또는 null(예정일 미정). 같은 곳에 놓으면 PATCH하지 않는다.
  ...
  const card = (d: CampaignDraftItem, fixedWidth = false) => {
    ...
    return (
      ...
      <div key={d.id} role="button" tabIndex={0} draggable
              ...
              className={`block cursor-grab rounded-md border border-l-4 px-2 py-1.5 text-left active:cursor-grabbing ${
                ...
                overdue ? 'border-y-red-200 border-r-red-200 bg-red-50 hover:bg-red-100/60' : 'border-y-x-border border-r-x-border bg-white hover:bg-x-hover'} ${
                unused ? 'opacity-60' : ''} ${dragId === d.id ? 'opacity-40' : ''}`}>
        <span className="line-clamp-2 text-content">{draftLabel(d).text}</span>
      ...
      {showUnscheduled && (
        <div {...dropProps(NONE, null)}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-ui text-x-muted">아래 달력의 날짜 칸으로 끌어다 놓으면 예정일이 정해져요</span>
          </div>
          {!collapsed && (
            grid.unscheduled.length > 0 ? (
              <div className="mt-2.5 flex flex-wrap gap-2">{grid.unscheduled.map((d) => card(d, true))}</div>
            ) : (
              // 끌고 있을 때만 나오는 빈 띠 — 여기 놓으면 예정일이 지워진다
