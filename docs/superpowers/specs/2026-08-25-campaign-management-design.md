...
**섹션 경계(QA 7라운드 08-26, koo 결정 B)**: 상세 영역은 연회색 배경(`bg-x-surface`) 위에 **섹션마다 흰 패널**(1px 테두리·둥근 모서리·20~24px 여백, 패널 간 20px) — 헤더 / 요약(한 패널 안 4칸, 안쪽 구분선) / 콘텐츠 진행 현황(툴바+표 또는 달력) / 인플루언서별 비용. 흰 배경에 여백만으로는 경계가 안 읽힌다는 피드백.
...
│ 예정 (1)           │ 섹션 제목 '콘텐츠 진행 현황 · N건' / 다음 줄 왼쪽: [ 표 | 주간 달력 ] + 단계 필터 칩 — 양쪽 보기 공통 │
│ 종료 (2) ▸접힘     │ ── 표: 예정일·콘텐츠·유형·인플루언서·비용·단계·게시물·성과 (밀림 먼저) │
│                    │ ── 달력: 상단 '예정일 미정' 카드 섹션(접기) + 주 행 격자(요일 헤더 1회, 카드 드래그로 예정일 변경) │
...
**콘텐츠 표** — 열 8개 고정: 예정일(110) · 콘텐츠(남는 폭 전부) · 유형(110) · 인플루언서(180) · 비용(120) · 단계(120) · **게시물(120)** · 성과(170). 표 위 도움말 문장 없음(QA 5라운드). (QA 1라운드: 유형은 스캔 대상이라 제목 보조줄이 아닌 열로; 단문/스레드 표기는 캠페인 관리에 중요치 않아 제거.)
- 기본 정렬 "밀린 것 먼저" → 예정일 오름차순 → 예정일 없음 마지막 → 미사용 맨 아래(흐리게). 정렬 드롭다운(예정일·단계·인플). 필터 칩(전체·준비 중(§2-4 정의)·전달됨·게시됨)은 표 안이 아니라 **상단 툴바**(보기 전환 옆)에 있고 표·달력 양쪽에 같은 필터가 적용된다(QA 4라운드 08-26).
...
- 단계 셀: `DraftStatusChip` + 게시됨(초록 ✓). 클릭 → 상태 변경(기존 칩 동작).
- 게시물 셀(QA 5라운드에서 단계 셀에서 분리): 미연결 → [게시물 연결] 버튼(트래킹 등록 모달, `draft_id` 자동 — 소수 케이스용, 자동 매칭은 백로그), 연결됨 → "✓ 연결됨".
...
**달력**(QA 6라운드 08-26 재설계 — 리서치 `calendar-view-ui-research-20260826.md` §3): **주를 행으로 아래로 쌓는 격자**(◀▶ 넘김 없음; 캠페인 기간이 걸치는 주 전부 + 예정일이 밖에 있는 주), 요일 헤더는 맨 위 1회, 셀마다 옅은 격자선·고정 행 높이(~150px, 카드 3개 초과는 "+N개" 펼침), 날짜는 셀 좌상단 작은 숫자, 오늘은 숫자 위 파란 원, 기간 셀은 옅은 틴트 + 시작/끝 캡 라인, 기간 밖 날짜는 흐리게(드롭 불가). 카드 = 흰 카드 + **왼쪽 4px 단계색 바**(칩 없음, 범례 점 행 하단), 밀림은 빨간 바 + "n일 지남". **'예정일 미정'은 격자 위 카드 섹션**(`예정일 미정 · N건`, 접기 토글·기억, 0건이면 숨김 — koo 결정: 초반 배정 작업 시점에 주 작업 대상이라 상단). 아래 구 설명은 참고용 — 7일 이하면 한 주, 넘으면 ◀ 8/24 주 ▶ 넘김(기본은 오늘이 든 주, 기간 밖이면 시작 주). 열 = 월~일 + "예정일 미정"(오른쪽, 점선; 문구 단일 소스 `NO_SCHEDULE_LABEL`). 주 라벨("8/31~9/6 주")·드래그 도움말은 두지 않는다(날짜 헤더가 이미 있음 — QA 4라운드). 카드 = 제목(2줄 말줄임) · @핸들 · 단계 칩. 밀린 카드 빨간 막대 + "n일 지남". 오늘 헤더 파란 강조. **카드 드래그 → 예정일 변경**(요일 ↔ 예정일 없음 포함, `PATCH scheduled_on`). 기간 밖 카드는 "기간 밖" 배지. 달력에서 새 원고 만들기는 백로그.
