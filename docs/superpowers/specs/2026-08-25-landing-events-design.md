...
| 표본 | 도착 <20 "아직 판단하기 일러요"(탭률 흐리게) / 20~49 "참고용" / 50+ 정상 — 임계값은 실데이터로 조정 |
...
| 표본 상태 | arrivals <20 → `early`("아직 판단하기 일러요", 탭률 흐리게) / 20~49 → `ref`("참고용") / 50+ → `ok`. 상수 `SAMPLE_EARLY=20`, `SAMPLE_REF=50` |
...
- 기간 `all | 7d | 30d | custom(from·to, 서울 날짜 YYYY-MM-DD, since 포함·until 다음 날 00:00 미포함)`. **도착·탭에만 적용**. 조회·클릭은 스냅샷이라 기간과 무관하게 최신값 — 화면 도움말에 명시. custom인데 날짜가 반쪽·역순이면 all로 되돌리고 응답 `range`로 알려준다(koo QA 08-26 추가).
- 캠페인 select에 **모든 캠페인**(`campaign=all`) — 링크 전부 합산, 미연결은 캠페인 조건 없이(koo QA 08-26 추가).
...
**필터 바** — 캠페인 select(`모든 캠페인` + `{utm_campaign} · {client_name}`) · 기간 세그먼트 `[전체 기간 | 최근 7일 | 최근 30일]`("캠페인 전체"는 모든 캠페인과 헷갈려 개명, koo QA 08-26) · `직접 지정 [시작일] ~ [종료일]`(`<input type=date>`, 둘 다 고르면 적용, 반쪽이면 힌트 "시작일과 종료일을 모두 고르면 그 기간으로 바뀌어요 · 지금은 전체 기간") · 우측 "{시작}~{끝} · 서울 기준"(전체면 첫 링크 생성일~오늘, 모든 캠페인이면 가장 이른 첫 링크).

**결정 카드 4**(숫자 26px, 라벨 13px, 보조 12px) — 순서는 사용자 흐름(koo QA 08-26):
1. 콘텐츠 조회 `180,000` — "본문 트윗 기준 · 클릭률 1.2%" + 11px "마지막 새로고침 기준"
2. 랜딩 도착(사람) `1,203` — "링크 클릭 2,220 중 54%가 페이지까지 왔어요"
3. LINE 탭 `143` — "도착 100명 중 11명이 눌렀어요"
4. **결정 카드**(왼쪽 3px 파란 테두리·연한 배경) — 라벨 "탭 상위 3개 콘텐츠의 탭 비중"은 값이 없어도 고정, 값 `71%`(없으면 "—") / 보조줄 제목 3개(없으면 "탭이 들어오면 어떤 콘텐츠가 만들었는지 여기 보여요")

**표 위** — `묶어 보기: [콘텐츠 | 인플루언서]` + 우측 정렬 캡션. 도움말 문구와 봇·스냅샷 각주 줄은 koo QA(08-26)로 제거 — 봇 제외 수는 화면에 두지 않고(백로그: 필요 시 툴팁), 스냅샷 시점은 조회 카드의 "마지막 새로고침 기준"과 펼침 각주가 맡는다.
...
**미연결** — 표 아래 `<details>` "▶ 링크와 연결되지 않은 랜딩 방문 N건 — 꼬리표 없음 a · 모르는 꼬리표 b". 펼치면 utm_content별 도착·탭 소표(상위 5 + 나머지).
...
UX 원칙 체크(AGENTS.md): 라벨은 이득 언어("아직 판단하기 일러요", "게시물 연결 전"), 내부 용어(Wilson·utm) 화면 노출 없음(utm_content 코드는 툴팁 데이터로만), 결과에 판단 서술(결정 카드·표본 배지), 라벨-값 일치(배지·정렬은 같은 `performanceJudgment` 함수), 비용 액션 없음(외부 호출 0).

## 게시물 표(`/tracking › 게시물`) 변경 — 역할

원고가 연결된 행의 **원고 열** 제목 아래 13px 역할 select(`본문` / `이어지는 본문` / `링크 댓글`, 자동 판정이면 흐리게·title "자동으로 판정했어요 — 눌러서 바꿀 수 있어요"). 클릭 → 3택 팝오버 → `PATCH /api/tracking/[id] {role}`. 자동 판정으로 되돌리기(null)도 팝오버에. 원고 미연결 행은 칩 없음(판정 근거가 없다).
