...
test('느슨한 조건 — 더 나오지 않는다고 알린다', () => {
  ...
  assert.equal(out[0].message, '이 컬럼은 좋아요 300 이상만 모으고 있어서 100으로 낮춰도 더 나오지 않아요');
});
...
test('반대 방향 — 항상 0건임을 더 강하게 알린다', () => {
  ...
  assert.equal(out[0].message, '이 컬럼은 좋아요 300 이상만 모으고 있어서 200 이하로는 한 건도 나오지 않아요');
});
...
test('여러 컬럼이면 요약한다', () => {
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'noEffect');
  assert.equal(out[0].message, '선택한 컬럼 중 2개는 좋아요 300 이상만 모아요');
});

test('인플루언서 컬럼은 수집 기준이 없어 경고 대상이 아니다', () => {
...
test('날짜 경계 — sinceDate와 정확히 같은 이전(before)은 항상 0건이라 경고한다', () => {
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'alwaysEmpty');
  assert.equal(out[0].message, '이 컬럼은 2026-06-01 이후만 모으고 있어서 2026-06-01 이전으로는 한 건도 나오지 않아요');
});
...
test('숫자 요약 — 기준이 다른 여러 컬럼은 가장 낮은 기준을 말한다', () => {
  const cols = [
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'noEffect');
  assert.equal(out[0].message, '선택한 컬럼 중 2개는 좋아요 100 이상만 모아요');
});
...
test('날짜 경계 — untilDate와 정확히 같은 이후(after)는 항상 0건이라 경고한다', () => {
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'alwaysEmpty');
  assert.equal(out[0].message, '이 컬럼은 2026-07-31 이전만 모으고 있어서 2026-07-31 이후로는 한 건도 나오지 않아요');
});
...
test('날짜 요약 — 기준이 다른 여러 컬럼은 가장 느슨한(이른) sinceDate를 말한다', () => {
  const cols = [
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'noEffect');
  // 두 컬럼 다 걸리므로(A2) 요약형이어야 한다 — 이 테스트 이름이 이미 "요약한다"였던 것과 달리
  // 이 문구는 이제껏 걸린 컬럼이 하나뿐일 때 쓰는 단수형이었다(날짜 축엔 요약형 자체가 없던 결함).
  assert.equal(out[0].message, '선택한 컬럼 중 2개는 2026-03-01 이후만 모아요');
});

// A2 — 컬럼 하나의 사정을 선택 전체의 결론처럼 말하지 않는다: 여러 컬럼이 선택 범위에 있으면
// 걸린 컬럼이 단 하나뿐이어도 "이 컬럼은 …"이 아니라 요약형을 써야 한다(그래야 나머지 컬럼까지
// 그 결론에 묶이지 않는다). 새 alwaysEmpty 요약 문구는 제품 담당자가 확정한 것 그대로다.

test('A2: 숫자 — 선택 범위에 여러 컬럼이 있어도 걸린 컬럼이 하나뿐이면 그 하나만 세어 요약한다 (noEffect)', () => {
  const cols = [A, col('b', 'B', { keywords: ['y'], minFaves: 50 })];
  const out = findConflicts([c({ value: '100' })], cols, []);   // 빈 배열 = 전체(2개), 걸리는 건 A 하나
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'noEffect');
  assert.equal(out[0].message, '선택한 컬럼 중 1개는 좋아요 300 이상만 모아요');
});
...
test('A2: 숫자 — 새 alwaysEmpty 요약 문구 (제품 확정)', () => {
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'alwaysEmpty');
  assert.equal(out[0].message, '선택한 컬럼 중 2개는 좋아요 300 이상만 모아서 그 컬럼에서는 한 건도 나오지 않아요');
});

test('A2: 숫자 — 선택 범위에 여러 컬럼이 있어도 걸린 컬럼이 하나뿐이면 alwaysEmpty도 요약한다', () => {
  const cols = [
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'alwaysEmpty');
  assert.equal(out[0].message, '선택한 컬럼 중 1개는 조회수 100,000 이상만 모아서 그 컬럼에서는 한 건도 나오지 않아요');
});

test('A2: 날짜(since) — 선택 범위에 여러 컬럼이 있어도 걸린 컬럼이 하나뿐이면 요약한다 (noEffect)', () => {
  const cols = [
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'noEffect');
  assert.equal(out[0].message, '선택한 컬럼 중 1개는 2026-06-01 이후만 모아요');
});

test('A2: 날짜(since) — 선택 범위에 여러 컬럼이 있어도 걸린 컬럼이 하나뿐이면 요약한다 (alwaysEmpty)', () => {
  const cols = [
    col('s', 'S', { keywords: ['x'], sinceDate: '2026-06-01' }),
    col('n', 'N', { keywords: ['y'] }),
  ];
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'alwaysEmpty');
  assert.equal(out[0].message, '선택한 컬럼 중 1개는 2026-06-01 이후만 모아서 그 컬럼에서는 한 건도 나오지 않아요');
});

test('A2: 날짜(until) — 선택 범위에 여러 컬럼이 있어도 걸린 컬럼이 하나뿐이면 요약한다 (noEffect/alwaysEmpty)', () => {
  const cols = [
  ...
  assert.equal(noEffect[0].message, '선택한 컬럼 중 1개는 2026-07-31 이전만 모아요');
  ...
  assert.equal(alwaysEmpty[0].message, '선택한 컬럼 중 1개는 2026-07-31 이전만 모아서 그 컬럼에서는 한 건도 나오지 않아요');
});

// 단수/요약 판정은 '검색 컬럼 수'가 아니라 '선택 범위 안 컬럼 수(검색+인플루언서)'로 해야 한다.
// 검색 컬럼 하나 + 인플루언서 컬럼 하나를 선택하면 검색 컬럼은 하나뿐이지만 범위 안 컬럼은 둘이라
// 단수형("이 컬럼은 …")이 아니라 요약형을 써야 그 컬럼들이 함께 보이는 표에서 대명사가 맞다.

test('혼합 선택(검색+인플루언서) — 숫자 noEffect: 검색 컬럼은 하나뿐이어도 범위 컬럼이 둘이면 요약형', () => {
  const w = watch('w', 'W');
...
test('혼합 선택(검색+인플루언서) — 숫자 alwaysEmpty: 요약형', () => {
  ...
  assert.equal(out.length, 1);
  assert.equal(out[0].kind, 'alwaysEmpty');
  assert.equal(out[0].message, '선택한 컬럼 중 1개는 좋아요 300 이상만 모아서 그 컬럼에서는 한 건도 나오지 않아요');
});
