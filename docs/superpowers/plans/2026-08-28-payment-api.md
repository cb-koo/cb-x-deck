...
```
`kstDateTime`이 `src/lib/datetime.ts:33`에 있음 — 시그니처 `(iso: string | null) => string` 확인 후 사용. `formatMoney(30000,'KRW')`가 `30,000원` 형식인지 `influencerPricing.ts`에서 확인(다르면 테스트 기대값을 실제 형식으로 맞춘다 — 함수를 새로 만들지 않는다).
