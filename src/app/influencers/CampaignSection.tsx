...
import { campaignStatus, CAMPAIGN_STATUS_LABEL, countsByTypeLabel, formatDateKo, type CampaignStatus } from '@/lib/campaignJudgment';
  ...
  return (
    ...
    <section className={PANEL}>
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        <InfoTip text="이 계정에 작업이 배정됐거나 추가 비용이 적힌 캠페인을 모아 보여줘요. 오른쪽 금액은 이 캠페인에서 이 사람의 작업 비용 + 추가 비용이에요(통화가 다르면 따로 보여요). 캠페인을 누르면 캠페인 화면이 열려요." />
        {campaigns.length > 0 && <span className="text-ui text-x-muted">· {campaigns.length}개</span>}
      </div>
      {campaigns.length === 0 ? (
        <p className="mt-1 text-ui leading-relaxed text-x-muted">아직 참여한 캠페인이 없어요 — 캠페인 화면에서 작업에 이 계정을 배정하면 여기 모여요.</p>
      ) : (
                ...
                <Link href={`/campaigns?id=${c.id}`}
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-ui text-x-muted">
                      {formatDateKo(c.startsOn)} ~ {formatDateKo(c.endsOn)} · 작업 {c.taskCount}건
                      {c.taskCount > 0 && <> ({countsByTypeLabel(c.countsByType)})</>}
                      {/* 돈이 붙었는데 작업이 없는 경우를 말로 드러낸다(캠페인 스펙 §4-1). 단, listInfluencerCampaigns는
                          taskCount를 셀 때 미사용 작업(붙은 원고가 미사용 + 미게시)을 뺀다 — 이 캠페인의 작업이 전부
                          미사용이고 추가 비용 행도 없으면 taskCount·subtotal이 둘 다 비어 "추가 비용만"이라 하면 거짓말이 된다.
                          그래서 비용이 실제로 있을 때만 "추가 비용만"이라 하고, 없으면 미사용 사실을 그대로 밝힌다. */}
                      {c.taskCount === 0 && (
                        formatMoneyBy(c.subtotal) !== '—' ? (
                          <span className="text-amber-800"> · 배정 작업 없음 — 추가 비용만</span>
                        ) : (
                          <span className="text-x-muted"> · 배정 작업 없음(미사용만)</span>
                        )
                      )}
                    </span>
                  </span>
