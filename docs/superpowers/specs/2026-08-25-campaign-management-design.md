...
```
┌ 캠페인 목록 280px ─┬ 상세 ──────────────────────────────────────────────┐
│ [+ 새 캠페인]      │ 헤더: 이름 · 상태 pill · 클라 / 라벨–값 목록(기간·유형·코드, 클릭 편집) / 메모 패널 │
│ 진행 중 (1)        │        [+ 원고 추가] [···]                              │
...
- 수정: 헤더는 **읽기 모드 라벨–값 목록**(기간 `8/31 월 ~ 9/6 일 · 7일`·유형·코드+복사) + 값 클릭 시 그 값만 편집(hover에 연필, Enter/이탈 저장·Esc 취소, 유형은 선택 즉시 저장·닫힘, 기간은 두 날짜라 이탈까지 열림). 메모는 라벨 붙은 패널(클릭 편집). 상시 폼 입력·긴 도움말 없음 — "기간 밖" 규칙은 기간 라벨 옆 ⓘ(QA 1라운드 08-26). 기간 변경으로 예정일이 기간 밖이 되면 막지 않고 카드·행에 "기간 밖" 표시.
