...
//
// koo 확인(2026-09-11): 미모드림 RT 3건의 이미지는 전부 RT 증빙이 맞다. 파일명에 인플 핸들이 적혀 있다.
// 세 장 모두 @chingcehansuki 의 9/9 미모드림 게시물을 리포스트한 화면이다(우리 작업 06 — target_task_id 로 이어져 있다).
//
...
// 실행: node --env-file=.env --import tsx scripts/attach-week2-proofs.ts [--apply]
...
const CONFIRMED_ON = '2026-09-09';                                   // 게시 확인일 — 슬랙 답글이 올라온 날

interface Task { no: number; campaign: string; handle: string; type: string; proof_file?: string; note: string }
const D = JSON.parse(readFileSync(new URL('../data-work/week2-normalized.json', import.meta.url), 'utf8')) as { tasks: Task[] };
...
async function main(): Promise<void> {
  ...
  console.log(`증빙 대상 ${src.length}건 · 올린 사람 ${actor.name} · 게시 확인일 ${CONFIRMED_ON}\n`);
  ...
  for (const t of src) {
    const [row] = await sql`
      select t.id, t.type, t.proof, to_char(t.posted_at,'YYYY-MM-DD') as posted_at,
       ...
       where c.name like '%_9월2주차' and lower(t.influencer_handle) = ${t.handle.toLowerCase()} and t.type = ${t.type}`;
  ...
  console.log(`\n넣을 값 — proof {url, by ${actor.id.slice(0, 8)}…, byName "${actor.name}", at 지금} · posted_at ${CONFIRMED_ON} · posted_source manual`);
  ...
  for (const j of jobs) {
    ...
    await sql`
      update campaign_task set
        ...
        posted_at = coalesce(posted_at, ${CONFIRMED_ON}::date),
        posted_source = coalesce(posted_source, 'manual'),
        updated_at = now()
      where id = ${j.taskId}`;
    console.log(`✓ @${j.t.handle} 증빙 ${path} · 게시 확인 ${CONFIRMED_ON}`);
  }

  const [x] = await sql`
    select count(*) as n from campaign_task t join campaign c on c.id = t.campaign_id
     where c.name like '%_9월2주차' and t.proof is not null`;
  console.log(`\n확인 — 증빙 있는 작업 ${x.n}건`);
  await sql.end();
}
