...
> **구현 후 정정(2026-08-01).** 아래 전체 코드는 **이 태스크 시점의 것이고, 지금 파일은 이보다 더 나아가 있다.**
> 리뷰에서 나온 두 가지가 이후 커밋(`87ad390`, `39851be`)으로 들어갔다 — 아래를 그대로 베끼면 되돌리는 셈이다:
> ① 조회를 기다리는 사이 누른 저장이 도착한 응답에 덮여 되돌아가던 문제 → `applySavedBy`가 `localSavedByRef`에
> 기록하고, 조회 결과의 저장자 목록을 그 값으로 덮어쓴다(설계 §C-5). ② 그 반대로 로컬에서 손대지 않았을 땐
> 서버 값을 **표 행에도** 전파해야 한다 — 안 그러면 다른 멤버의 저장이 팝업에만 반영되고 표는 낡은 채로 남는다.
> 그 밖에 상단 바 `sticky`, 컬럼 알약 `truncate`, 날짜 줄 `ml-auto`, 재시도 시 로딩 상태 복귀가 함께 들어갔다.
