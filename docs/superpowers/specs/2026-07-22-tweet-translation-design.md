...
- `TweetCard`:
  ...
  - **번역 대상은 본문(+인용 본문)뿐**. 저자명·@핸들은 식별자라 원문 유지(멘션과 동일 취급),
    이미지 속 텍스트(OCR)는 번역하지 않음 — 필요 시 카드에 기대 한 줄로 명시.
...
- `Column` 헤더: **`전체 번역`** 버튼 → 로드된 트윗 전부 번역(진행 표시 후 각 카드 반영),
  ...
  - **기대 설정 한 줄 도움말**(AGENTS #2/#6): "지금 보이는 트윗을 한국어로 (몇 초 걸릴 수 있어요)".
    비용/시간 유발 액션임을 행동 전에 알림. 필터(전체↔NEW만) 상태와 무관하게 "로드된 트윗"이 대상.
...
- **보관함(라이브러리 `CandidateCard`) 번역 — fast-follow (사용자 결정 2026-07-22)**: 초기는 덱
  (`TweetCard`/`QuotedCard`/`Column`)만. 백엔드(모델·스토어·라우트·캐시)는 tweet_id 기준이라 전부
  공유되므로, 덱에서 검증 후 `CandidateCard` UI 배선만 추가하면 됨(저비용). 덱에서 번역·캐시된 트윗은
  보관함에서도 즉시 히트.
- 원문 완전 대체 모드
- 역번역 QA / 품질 점수
