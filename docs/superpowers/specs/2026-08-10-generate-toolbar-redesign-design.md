...
- 팝오버: 프리셋 버튼 4개(현재 값 하이라이트) + 구분선 + "직접 지정" 라벨 + 네이티브 `<input type="date">` 2개(시작·끝). 프리셋 클릭=즉시 적용+닫힘, 날짜는 각 필드 변경 즉시 적용(단측 범위 정식 지원 — applyPeriod 참조, 팝오버는 유지되어 연달아 조정 가능), 바깥 클릭·Esc 닫힘.
