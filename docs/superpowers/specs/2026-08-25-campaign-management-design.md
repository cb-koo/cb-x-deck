...
| `kind` | text null | **UI에서 제거(koo 결정 08-26)** — 캠페인 유형은 콘텐츠 표의 유형 열(원고 비용 유형)로 판단하고, 시딩 같은 성격은 캠페인명으로 표현. 컬럼·API 검증은 남아 있으나 화면에서 읽고 쓰지 않음(정리 마이그레이션은 후속) |
...
```
┌ 캠페인 목록 280px ─┬ 상세 ──────────────────────────────────────────────┐
│ [+ 새 캠페인]      │ 헤더: 이름 · 상태 pill · 클라 / 속성 블록 2칸(기간·코드, 클릭 편집) / 메모(있을 때만) │
│ 진행 중 (1)        │        [+ 원고 추가] [···]                              │
...
- [+ 새 캠페인] 모달: 클라이언트(필수, 명부 select) → 기간(기본 다음 월~일, DateOnly 두 칸) → 이름(자동 제안, 수정) → 영문 코드(자동 제안, 전용 문구로 검사) → 메모. (유형 칸 제거 — koo 결정 08-26) 만들면 그 캠페인이 선택된 상세로.
- 수정: 헤더는 제목 줄 + **속성 블록 2칸**(세로 구분선, 칸마다 13px 라벨 위·15~16px 굵은 값 아래: 기간 `8/31 월 ~ 9/6 일 · 7일` / 코드+복사 — 유형 칸은 제거 — QA 3라운드 결정 A: 글자만 한 줄이면 값 시작점을 눈이 못 찾음; 좁으면 칸 줄바꿈) — 값 클릭 시 그 값만 편집(hover에 연필, Enter/이탈 저장·Esc 취소, 유형은 선택 즉시 저장·닫힘, 기간은 두 날짜라 이탈까지 열림). **메모는 값이 있을 때만** 그 아래 패널로 표시, 추가는 [···] 메뉴 "메모 추가"(빈 칸 상시 노출 금지 — QA 2라운드 08-26 결정 A; 컬럼은 유지). 상시 폼 입력·긴 도움말 없음 — "기간 밖" 규칙은 기간 라벨 옆 ⓘ(QA 1라운드 08-26). 기간 변경으로 예정일이 기간 밖이 되면 막지 않고 카드·행에 "기간 밖" 표시.
