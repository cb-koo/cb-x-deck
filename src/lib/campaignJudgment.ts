// 캠페인 판정의 순수 함수 — 작업 단계·밀림·기간 밖·캠페인 상태·준비 중·요약·정렬·인플 목록·이름/코드 제안·주 계산.
// 서버 요약(campaignStore)과 클라 표시(/campaigns·TaskTable·달력)가 같은 함수를 쓴다(influencerJudgment 선례) —
...
import { sumMoney, mergeMoney, type ExtraCost, type MoneyByCurrency, type TaskCost } from './campaignCost.ts';
...
// 유형은 표시·필터용 속성 — 로직 분기는 없다(작업 유형이 비용·단계를 정한다, 스펙 §2-1)
...
// 준비 중 = 아직 게시 전 단계 묶음. STATUS_LABEL에 없는 합성 라벨이라 정의를 여기 한 곳에 둔다 —
// 칩·카드·필터가 전부 같은 말을 쓴다(§2-4). 판정은 작업 기준(isTaskPreparing).
...
export const STAGE_FILTER_LABEL: Record<StageFilter, string> = {
  all: '전체', preparing: PREPARING_LABEL, delivered: STATUS_LABEL.delivered, published: '게시됨',
};
