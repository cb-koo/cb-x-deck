...
- **오류와 성공은 동시에 뜨지 않는다.** 원칙 4가 금지하는 모순이라 표시가 아니라 로직에서 막는다. 두 경로 모두 닫는다:
  - 입력이 바뀌면 지난 오류를 지운다(`onChange`에서 `setErr('')`) — 실패 후 올바른 링크를 붙여넣는 경우.
  - 성공 문구 자체를 `!err`로 가린다 — 서버가 `계정을 찾을 수 없음`을 돌려줬을 때는 **입력이 그대로**라서 오류를 지울 계기가 없다. 이걸 빼면 오타 핸들 하나로 같은 핸들에 초록 `✓`와 빨간 오류가 함께 남는다(더 흔한 경로다).
