...
export function FilterChips({
  ...
  conflicts: Conflict[];
  columnNames: string[];
  // 컬럼 선택이 있는지의 진실은 activeColumnIds다 — columnNames는 제목을 표시용으로 붙인 파생값이라
  // 제목이 빈 문자열로 풀리면(오늘은 컬럼 생성이 막지만 PATCH는 안 막는다) 줄·지우기 버튼이
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {hasColumnFilter && (
          <span className={`${chip} border-x-border-strong`}>
            컬럼 {columnSelectionLabel(columnNames)}
            <button type="button" aria-label="컬럼 선택 지우기" title="컬럼 선택 지우기" onClick={onClearColumns} className={del}>✕</button>
          </span>
        )}
        {ready.map((c) => {
