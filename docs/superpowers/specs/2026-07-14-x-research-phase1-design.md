...
기존 검색 컬럼을 "지표 기반 벤치마크 발굴" 도구로 굳히고(엔게이지먼트 연산자 확장 + 실측 가드레일), 실측으로 확인된 검색 노이즈를 담당자가 치워 주간 뷰를 깨끗하게 유지하며(그 저장/버림이 부수적으로 향후 자동 필터의 신호가 됨), 일본 규제 리스크에 가벼운 경고를 붙인다. 주간 루틴 사용이 무게중심.

**성공 기준**: "주간 루틴을 새로 만든다"가 아니라(그 골격은 기존 저장 컬럼 + NEW 배지가 이미 제공) **"발굴 필터·큐레이션 품질이 나아졌는가"**로 판정한다.

## 아키텍처 위치 (경계)

**X 콘텐츠 리서치의 본체는 `덱`이다.** 덱의 검색 컬럼(벤치마크 발굴)·워치리스트(계정 관찰)·보관함(큐레이션)이 곧 X 리서치 작업장이다. `리서치` 페이지는 **온램프(뭘 볼지 모를 때 검색어를 찾아 덱 컬럼을 생성)와 종합 리포트 전용**이며, 현재는 exa 웹 트랙만 있고 **Phase 1에서 변경하지 않는다.** 구분선: "이미 뭘 볼지 안다"=덱 / "뭘 볼지부터 찾는다·기간을 요약한다"=리서치 페이지. 따라서 Phase 1의 모든 변경은 덱과 그 하위 컴포넌트에서 일어난다.
...
### 3. 버림(dismiss) — 워크스페이스 단위, 숨김 기본·복구 가능

> **정당화(순서 주의)**: 버림의 1차 목적은 **지금 당장의 노이즈 제거**다 — 실측에서 브로드 키워드 고성과 트윗의 상당수가 미용실 농담·만화 등 벤치마크 무관 콘텐츠였다. 담당자가 이를 치우면 주간 뷰가 깨끗해지고, 버림이 새로고침 넘어 지속되므로 다음 주 NEW 배지도 깨끗해진다. 저장(양성)/버림(음성) 신호가 향후 자동 관련성 필터의 학습 데이터가 되는 건 **부수 효과**이지 이번 구현의 정당성이 아니다(자동 필터는 신호가 쌓인 뒤 별도 결정).
