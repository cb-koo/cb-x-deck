...
통화는 수단마다 다를 수 있다(한국 계좌 ₩, 일본 계좌·PayPay ¥, PayPal은 둘 다). **이 통화는 "인플이 받는 돈의 통화(지급 통화)"** 다 — 캠페인·단가의 원화 기준과는 다른 질문이라 새 수단 기본값은 ¥(실데이터 91건 중 84건). 원화→엔화 환산(10원 = 1엔)은 결제 수단이 아니라 결제 요청(payment-data)이 요청 시점 환율 스냅샷과 함께 원화·지급 통화 금액을 **둘 다** 저장·송신한다. 수수료(`fee`)는 지급 통화 금액에 적용한다.
...
- 표시 도우미: `PAYMENT_TYPE_LABEL`(PayPal/PayPay/계좌이체), `formatFee(fee, currency)`("송금 수수료 CB 부담 · 5%" / "송금 수수료 CB 부담 · 165엔"), `getDefaultPaymentMethod(list)`.
...
- **추가/수정 폼**(인라인, 카드 자리에서 펼침, 한 번에 하나만): 유형 select(먼저) → 유형별 칸: 공통 `수취인명`(계좌이체는 `예금주`), `통화`(PayPay면 ¥ 고정·비활성), PayPal `이메일`, PayPay `수취 식별 정보(선택)`, 계좌이체 `은행`·`지점(선택)`·`계좌번호`. `송금 수수료` select: `인플 부담` / `CB 부담 (비율)` (+ 비율 % 입력, 기본 5) / `CB 부담 (고정액)` (+ 고정액). `메모(선택)`. 추가 폼에는 `기본 수단으로` 체크(첫 수단이면 체크 고정·비활성 + "첫 수단은 기본이 돼요"). `저장`/`취소`. 저장 중 버튼 비활성·`저장 중…`.
...
- **UX 원칙**: ① 라벨은 사용자 말(결제 수단·수취인·인플 부담/CB 부담 — `grossUp` 같은 내부어 노출 없음) ② 패널 도움말 1줄 + 폼 안 힌트(첫 수단은 기본이 돼요) ③ 판단 서술은 해당 없음(입력 화면) ④ `기본` 배지는 서버 `isDefault`에서만 파생 ⑤ 통화 기호 옆에 뜻 없음이 자연스러움 ⑥ 비용 유발 없음.
...
- 2026-08-27: 새 수단 기본 통화 ₩→¥, 라벨 '지급 통화', 환산 규칙 도움말 한 줄(koo 논의: 캠페인은 원화 관리, 정산 시 10원=1엔 환산, 정산 프로덕트에 두 통화 모두 송신 — 환산은 payment-data 책임).
- 2026-08-27: 수수료 옵션명을 `인플 부담 / CB 부담 (비율) / CB 부담 (고정액)`으로(koo 결정) — '실수령 보장'은 결과와 주체가 섞인 개념어라 폐기. 카드·타임라인 표시(`formatFee`)도 같은 말로 통일.
