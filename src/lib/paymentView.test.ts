...
test('2) 요청 스냅샷이 있으면 그것이 우선(단계 정산·완료)', () => {
  const v = buildPaymentView({
    ...
    roster: { id: 'inf1', methods: [pm({ email: 'new@x.com' })] },
    chosenId: null,
  });
...
test('3) 명부 밖 / 수단 없음 / 기본 수단 + 고를 목록 + 고른 수단·fallback', () => {
  assert.deepEqual(buildPaymentView({ request: null, roster: null, chosenId: null }), { state: 'notInRoster' });
  assert.deepEqual(buildPaymentView({ request: null, roster: { id: 'inf1', methods: [] }, chosenId: null }), { state: 'none', influencerId: 'inf1' });
  const methods = [pm({ id: 'a', isDefault: false, type: 'paypay', currency: 'JPY', email: undefined, fee: { mode: 'grossUp', percent: 3 } }), pm({ id: 'b' })];
...
after(async () => {
  await sql`delete from campaign_task where campaign_id in (select id from campaign where name = ${P})`;
  await sql`delete from campaign where name = ${P}`;
...
test('4) loadPaymentView: 인플 1명 + 기본 수단 1개 → ok, 대문자 조회도 ok, 없는 핸들 → notInRoster', async () => {
  ...
  assert.ok(v1.state === 'ok' && v1.influencerId === row.id && v1.choices.length === 1);
});

test('5) loadPaymentView: 작업이 고른 수단(payment_method_id) — 두 번째 수단을 고르면 그것, 지워진 id면 기본 + fallback', async () => {
  const handle = P + 'Chosen';
  const { row } = await createInfluencer(sql, { handle, createdBy: null });
  await updatePaymentMethods(sql, row.id, { kind: 'add', input: paypalInput('デフォルト') }, null);
  const afterAdd = await updatePaymentMethods(sql, row.id, { kind: 'add', input: { type: 'paypay', holder: 'サブ', currency: 'JPY' } }, null);
  const second = afterAdd.paymentMethods.find((m) => !m.isDefault)!;

  const campaign = await createCampaign(sql, {
    clientId: null, clientName: null, name: P, nameEn: P, startsOn: '2026-01-01', endsOn: '2026-01-31', kind: null, note: '', createdBy: null,
  });
  const [chosenTask] = await createTasks(sql, campaign.id, {
    type: 'post', targetTaskId: null, targetTweetUrl: null, draftId: null, scheduledOn: null, visitOn: null, note: '', createdBy: null,
    items: [{ handle, cost: null, paymentMethodId: second.id }],
  });
  const [goneTask] = await createTasks(sql, campaign.id, {
    type: 'post', targetTaskId: null, targetTweetUrl: null, draftId: null, scheduledOn: null, visitOn: null, note: '', createdBy: null,
    items: [{ handle, cost: null, paymentMethodId: 'no-such-id' }],
  });

  const chosenView = await loadPaymentView(sql, { handle, taskId: chosenTask.id });
  assert.ok(chosenView.state === 'ok' && chosenView.label.startsWith('PayPay') && chosenView.fallback === false);

  const goneView = await loadPaymentView(sql, { handle, taskId: goneTask.id });
  assert.ok(goneView.state === 'ok' && goneView.label.startsWith('PayPal') && goneView.fallback === true);
});
