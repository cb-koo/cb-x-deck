...
// repostMessage: 링크를 바꿀 수 없는 자리(게시 확인 뒤의 게시 칸)에서 "바꿔 주세요" 대신 쓸 사실 문구.
export function TargetPreview({ state, preview, loading, failed, lines, repostMessage }: {
  ...
  if (failed || !preview || preview.kind !== 'ok') {
    const msg = preview?.kind === 'repost' ? (repostMessage ?? '리포스트 링크예요 — 원본 게시물 링크로 바꿔 주세요') : '게시물을 불러올 수 없어요';
    return (
      <div className={box}>
        {msg}<br />
