...
- **원고를 통해 들어오는 핸들은 막지 않는다(koo 09-23 결정):** ① 새 작업의 '있는 원고 고르기'가 원고 주인으로 인플 칸을 채우는 길(`pickForForm`→`formHandleFill`, FlowDetail:887) ② 원고 모드의 원고 카드 인플 칩(`onAssignInfluencer`, FlowDetail:695-720 — 자유 입력) ③ 인플 미정 작업에 원고를 붙일 때 서버가 원고 주인으로 채우는 것(`attachDraft`의 coalesce, campaignTaskStore:222). 셋 다 배정은 그대로 두고, 결과가 명부 밖이면 인플 칸이 위 '예전에 명부 없이 배정된 작업'과 같은 `명부에 없음 [명부에 등록]` 상태로 보인다. 새 작업 폼에서는 [만들기] 전이라도 같은 표시 + 등록 버튼(등록만, 배정은 이미 채워져 있다).
...
- 서버에서 명부 밖 핸들 거부 + 다른 입구 게이팅: BulkCreateDialog(한 번에 만들기), 원고 카드 → 작업 만들기, 기존 `/campaigns` 화면(TaskAddModal).
- 정산 후보 화면에 "고른 수단 삭제됨" 표시.
- 명부 밖 핸들 8건 정리(koo 판단).
