...
| 단계 | 내용 | 서버·DB |
|---|---|---|
| **1단계 — 보이는 것** | §3 헤더 · §4 섹션 상자 · §5 단계·유형 칸 · §6 인플 표시 · §7 원고 칸 · §8-1 결제 수단·수수료 **표시** | 조회만 늘림(마이그레이션 없음) |
...
- `AI로 만들기`(파란 테두리·연파랑 바탕으로 강조) / `직접 쓰기` / `있는 원고 고르기`, 각 버튼 아래 작은 보조 글: `시안 여러 개` / `빈 칸에서` / `N개 있어요`(pickCount, 못 읽었으면 생략).
- 아래 도움말 "인플루언서가 직접 쓰면 비워 둬요"(지금 문구), 409 경고(`다른 작업에 붙었어요 — 다시 고르기`)도 지금 자리·톤 그대로.
