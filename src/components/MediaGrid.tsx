...
// compact: 보관함 밀도 모드 — 단일 이미지 세로 상한 208px(기본 384px).
// 2장 이상은 밀도 모드에서도 정사각 유지 — 16:9 캡을 시도했다가 이미지가 어색하게 잘려 회귀(08-15 QA).
      ...
      {imgs.map((m, i) => (
        <div key={i} className={`${imgs.length === 3 && i === 0 ? 'row-span-2' : ''}${renderOverlay ? ' relative' : ''}`}>
          ...
          <img src={m.url} alt="" loading="lazy"
               className={`h-full w-full object-cover ${imgs.length === 1 ? (compact ? 'max-h-52' : 'max-h-96') : 'aspect-square'}`} />
          {renderOverlay && (
