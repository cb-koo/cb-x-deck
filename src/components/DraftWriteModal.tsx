...
export function DraftWriteModal({ clientId, clientName, procedureNames, procedureIds, taskId, taskLabel, onClose, onSaved }: {
  ...
  // 클라이언트도 작업도 없으면 아무것도 그리지 않는다: '연결 없음'을 말해봐야 지금 할 일이 달라지지 않는다.
  const scope = [taskLabel ? `${taskLabel} 작업` : null, clientName, ...procedureNames].filter(Boolean).join(' · ') || null;
  ...
  async function save() {
    ...
    const r = await apiFetch('/api/drafts/manual', {
      ...
      body: JSON.stringify({
        ...
        ...(taskId ? { taskId } : {}),   // 없으면 키 자체를 싣지 않는다(undefined = 붙는 작업 없음)
      }),
    });
    setSaving(false);
