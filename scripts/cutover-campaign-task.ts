...
(async () => {
  ...
  const sql = getSql();
  try {
    // 039 뒤(draft.campaign_id가 없음)에 다시 돌려도 컬럼 오류 대신 '이미 끝남'으로 — cutoverDraftsToTasks와 같은 가드
    ...
    if (Number(has[0].n) === 0) { console.log('draft.campaign_id 컬럼이 없어요 — 039이 적용된 뒤라 이관은 이미 끝났어요'); return; }
