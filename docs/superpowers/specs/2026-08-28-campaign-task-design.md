...
- **인플루언서 칸**(koo QA 결정 08-28) — **투고·인용RT·방문협찬은 1명(단일 선택)**, **RT만 여러 명**(칩, 사람 수만큼 작업). 근거: 슬랙 실무에서 원고 1건 = 인플 1명(거절 시 다음 사람에게 재배정), RT는 원고 없이 주당 13~20건이라 묶음 입력이 필요. 유형을 RT→그 외로 바꾸면 첫 사람만 남기고 "@b, @c는 빠졌어요" 안내. 원고 라디오는 항상 활성(제약을 설명 문장으로 막지 않고 구조로 표현). 발행 유형엔 **[만들고 하나 더]**(유형·대상·날짜 유지, 인플·원고 초기화). 이전 안(N명 + 원고 비활성)과 '사람별 카드에 원고 칸' 안은 안정화 뒤 재검토.
- **사람별 줄(koo QA 1 반영 08-28)** — 금액과 함께 **게시 예정일(방문협찬은 방문일도)을 사람별로** 받는다. 상단 날짜는 '(전체) + [모두에게 적용]' 도우미(누를 때만 전파). API `influencers[].scheduledOn/visitOn`.
