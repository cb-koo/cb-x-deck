...
export function PriceProfileDialog({ scenario, handle, type, profile, entered, onAnswer }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-6" onClick={() => onAnswer(false)}>
      <div role="dialog" aria-modal="true" aria-label="프로필 단가" className="w-full max-w-[420px] rounded-2xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 flex justify-end gap-2.5">
          <Button onClick={() => onAnswer(false)} className="h-10 px-4 text-content">이 작업만</Button>
          <Button variant="primary" onClick={() => onAnswer(true)} className="h-10 px-4 text-content">
