...
| 상태 | 표시 |
|---|---|
| 배정됨 | `@hadakan__ ⌄` — 흰 바탕 + 실선 테두리 |
| 미배정 | `+ 인플루언서` — 흰 바탕 + **점선** 테두리, 낮은 대비 |

값의 유무를 실선/점선으로 말한다. 점선은 "비어 있는 칸"이라는 뜻이라 미배정이 *채우는 자리*로 읽힌다. 의미색은 상태 칩이 독점하므로 여기는 무채색이다.

미배정 표시는 "없는 데이터"가 아니라 **행동 손잡이**라서 자리를 만든다 — 상태 칩이 늘 그 자리에 있는 것과 같다. 다만 조용해야 한다.

### 컨트롤 규격 (상태 칩과 공유)

> **2차 수정.** 처음엔 상태 칩에 톤을 맞춰 `text-caption`(11px)으로 만들었는데 "너무 작아서 눈에 들어오지 않는다"는 피드백을 받았다. 상태 칩도 같은 문제였다 — **바로 옆의 읽기용 메타 글자와 크기도 색도 같아서**, 누를 수 있는 것이라는 신호가 하나도 없었다.

이 줄의 컨트롤 둘은 **같은 규격**을 쓴다: `text-ui`(13px) · `h-8`(32px) · `rounded-lg` · 테두리 · `px-2.5`.

- **테두리가 어포던스를 진다.** 채움만으로는 배지와 구분되지 않는다.
- **상태 칩은 옅은 색 채움을 유지하고 같은 계열 테두리를 더한다.** 채움을 색 점으로 줄이면 컨트롤로는 또렷해지지만 여러 장을 훑을 때 진행 상황이 멀리서 안 읽힌다 — 색 코딩이 하던 일을 잃는 게 더 비싸다.
- **읽기용 메타는 11px 회색으로 남긴다.** 대비는 컨트롤을 키워서가 아니라 *둘의 차이*로 생긴다.

이 규격은 `DraftTable`의 상태 칩에도 그대로 적용된다(같은 컴포넌트). 표의 행 높이가 늘지만, 안 보이던 문제는 표에서도 같았다.
...
**이 줄에서 시술명을 뺀다.** 상단 오른쪽 `보톡스 · 단문`과 하단 근거 풋터의 `클라이언트 · 보톡스`에 시술명이 한 카드에 두 번 나오고 있었다. 커진 컨트롤 바로 옆의 중복 글자가 대비를 갉아먹으므로 상단에는 형식(`단문`/`스레드`)만 남기고 시술은 풋터로 일원화한다 — 정보는 하나도 잃지 않고, 이 줄은 "내가 정하는 것"만 남는 줄이 된다.

**자리에 대한 판단.** 상단 스트립이 맞다. 이 줄의 존재 이유가 "카드를 열지 않고 훑도록"이고, 상태와 배정이 정확히 훑으며 확인하는 값이다. 대안은 둘 다 나쁘다 — 본문 아래 액션 행은 X 액션 바를 미러링하는 자리라 관리용 컨트롤을 섞으면 이 제품이 일부러 지키는 X 화면 재현이 깨지고, 그 행은 *원고 텍스트에 하는 일*이지 *원고에 대한 사실*이 아니다. 하단 도구층은 검수·근거를 읽는 층이고 스레드가 길면 한참 아래라, 상태를 바꾸러 스크롤하게 만든다.
