...
**이 문서의 태스크별 코드 블록은 실행 시점 기준이다. 최종 코드는 아래 수정이 더해졌으니, 규칙·문구의 authority는 설계 문서(`docs/superpowers/specs/2026-07-28-influencer-column-url-input-design.md`)를 본다.** 최종 브랜치 리뷰가 찾은 것:

1. **(Important) PATCH 건너뛰기 경로에서 `userId`도 되돌린다.** `updateColumn`이 `config`를 통째로 교체하고 트윗 조회는 `userId`로 키를 잡으므로, `handle`만 되돌리면 클라이언트가 보낸 엉뚱한 `userId`가 저장돼 제목과 실제 수집 계정이 어긋난다. 대소문자 무시 비교로 이 경로가 넓어진 탓에 생긴 구멍이다.
2. **(Important) 오류와 성공이 동시에 뜨지 않게 두 경로를 막는다.** `onChange`에서 `setErr('')`(실패 후 다시 붙여넣는 경우) + 성공 문구를 `!err`로 가리기(서버가 "계정을 찾을 수 없음"을 돌려줬을 때는 입력이 그대로라 지울 계기가 없다 — 오타 핸들이라는 더 흔한 경로). 첫 수정만으로는 후자가 남아 재리뷰에서 다시 걸렸다. AGENTS UX 원칙 4.
3. **예약 경로에 `statuses`·`communities` 추가.** 옛 트윗 영구링크·커뮤니티 링크가 핸들 형식과 겹쳐 유료 콜을 낭비했다.
4. **호스트 허용 목록 회귀 테스트 추가** — `x.com.evil.com` / `x.com@evil.com` / `m.evil.com`은 `invalid`, `evil.com@x.com`은 통과.
5. **성공 문구를 `✓ @핸들 계정을 추적할게요`로.** `을/를`은 앞 음절 받침에 따라 달라지고 붙여 쓰는 조사라, 임의의 라틴 핸들 뒤에는 어떤 조사도 항상 맞을 수 없다. `aria-live="polite"`도 함께 붙였다.
6. **`[id]/route.ts` 주석 정정** — `@handle ↔ handle`은 원래 문제가 아니었다(기존 코드도 비교 전에 `@`를 떼어냈다). 실제 원인은 대소문자 차이와 URL 표기뿐.

최종 검증: 테스트 222 통과/0 실패, `tsc` 클린, 린트 `main`과 증감 0.
